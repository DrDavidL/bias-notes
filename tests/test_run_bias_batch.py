...
class RunBiasBatchTests(unittest.TestCase):
    def test_select_rows_is_seeded_for_reproducible_random_samples(self):
        ...
        first, first_mode = select_rows(
            df, charts_to_process=5, selection="random", seed=42
        )
        second, second_mode = select_rows(
            df, charts_to_process=5, selection="random", seed=42
        )
    ...
    def test_select_rows_can_take_first_n_rows(self):
        df = pd.DataFrame(
            {
                "unique_id": list(range(10)),
                "note_text": [f"note {i}" for i in range(10)],
            }
        )

        selected, selection_mode = select_rows(
            df, charts_to_process=3, selection="head", seed=42
        )
